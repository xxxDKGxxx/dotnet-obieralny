...
To update the database use this command from the Web project folder (replace `LoanHub.Backend` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../LoanHub.Backend.Infrastructure/LoanHub.Backend.Infrastructure.csproj -s LoanHub.Backend.Web.csproj
```
